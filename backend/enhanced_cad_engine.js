...
class EnhancedCADEngine:
    ...
    def _generate_generic_solid(self, dimensions: Dict[str, Any]) -> Dict[str, Any]:
        """Generate generic solid for unknown component types"""
        values = dimensions.get('values', [50, 50, 10])
        width = values[0] if len(values) > 0 else 50
        height = values[1] if len(values) > 1 else 50
        depth = values[2] if len(values) > 2 else 10
        
        return self._generate_plate({'values': [width, height, depth]})
